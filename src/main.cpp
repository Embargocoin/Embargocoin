...
const string strMessageMagic = "Embargocoin Signed Message:\n";
...
/** Amount of embargocoins spent by the transaction.
...
static const int64 nBaseSubsidy = 500 * COIN;
static const int64 nMinSubsidy = 1 * COIN;

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = nBaseSubsidy;
	if ((nHeight >= 1) && (nHeight <= 251))
	{
		nSubsidy = nStartSubsidy;
	}
	if ((nHeight >= 252) && (nHeight <= 503))
	{
		nSubsidy = nStartSubsidy * 2;
	}
	if ((nHeight >= 504) && (nHeight <= 755))
	{
		nSubsidy = nStartSubsidy * 3;
	}
	if ((nHeight >= 756) && (nHeight <= 1007))
	{
		nSubsidy = nStartSubsidy * 4;
	}
	if ((nHeight >= 1008) && (nHeight <= 1259))
	{
		nSubsidy = nStartSubsidy * 5;
	}
	if ((nHeight >= 1260) && (nHeight <= 1511))
	{
		nSubsidy = nStartSubsidy * 6;
	}
	if ((nHeight >= 1512) && (nHeight <= 1763))
	{
		nSubsidy = nStartSubsidy * 7;
	}
	if ((nHeight >= 1764) && (nHeight <= 2015))
	{
		nSubsidy = nStartSubsidy * 8;
	}
	if ((nHeight >= 2016) && (nHeight <= 2267))
	{
		nSubsidy = nStartSubsidy * 9;
	}
...
{
        static const int64 BlocksTargetSpacing = 20 * 60; // 20 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (TestNet()) {
                ...
                if (pindexLast->nHeight+1 >= 600) { DiffMode = 2; }
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmbargocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmbargocoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("EmbargocoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running EmbargocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmbargocoinMiner terminated\n");
        throw;
    }
}
